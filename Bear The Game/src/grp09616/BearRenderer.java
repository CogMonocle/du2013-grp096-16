...
public class BearRenderer
{
	...
	private static final double BACKGROUND_SCALE_MODIFIER = 0.9;
	...
	private static final int BACKGROUND_LAYERS = 1;
	...
	public static void renderMenu()
	{
		...
		BearRenderer.drawRect(0, 0, BearGame.WINDOW_WIDTH,
				BearGame.WINDOW_HEIGHT, menuTexture);
	...
	public static void loadTextures()
	{
		...
		{
			groundTextures[i] = TextureManager
		...
		{
			backgroundTextures[i] = TextureManager
		...
		// "menu/background.png");
		energyPipTexture = TextureManager.loadTextures(BearGame.TEXTURE_PATH + "gui/enPip.png");
	...
	public static void drawBackground(BearWorld w)
	{
		...
		for (int i = 0; i < BACKGROUND_LAYERS; i++)
		{
			...
			} else
			{
				...
				drawRect(shift, 0, BearGame.WINDOW_WIDTH * scaleMod,
						BearGame.WINDOW_HEIGHT, backgroundTextures[(i % 2) + 1]);
				drawRect(backgroundWidth + shift, 0, BearGame.WINDOW_WIDTH
						* scaleMod, BearGame.WINDOW_HEIGHT * scaleMod,
						backgroundTextures[(i % 2) + 1]);
			}
	...
	public static void drawGround(BearWorld w)
	{
		...
		{
			while (groundDisplacement > (width = TextureManager.getTexture(
		...
		while ((totalGroundPieceWidth - groundDisplacement) < BearGame.WINDOW_WIDTH)
		{
			...
			totalGroundPieceWidth += TextureManager.getTexture(textPiece)
					.getImageWidth();
		}
